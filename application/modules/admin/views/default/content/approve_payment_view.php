           ...
           <tbody>
        	...
        	<?php foreach($trans->result() as $row):  ?>
               ...
               <tr>
                  ...
                  <td data-title="<?php echo lang_key('request_date');?>" class="numeric"><?php echo translatedDate(strtotime($row->request_date));?></td>
                  
                  <td data-title="<?php echo lang_key('expiration_date');?>" class="numeric"><?php echo translatedDate(strtotime($row->expiration_date));?></td>
                      ...
                      <?php if(is_admin()){?>
                        <?php if($row->is_active==2){?>
                            <?php if($row->payment_type=='post') {?>
                              <li><a href="<?php echo site_url('admin/content/approveposttransaction/'.$row->unique_id);?>"><?php echo lang_key('approve');?></a></li>
                            <?php }?>
                            <?php if($row->payment_type=='post_renew') {?>
                              <li><a href="<?php echo site_url('admin/content/approveposttransaction/'.$row->unique_id);?>"><?php echo lang_key('approve');?></a></li>
                            <?php }?>
                            <?php if($row->payment_type=='feature') {?>
                              <li><a href="<?php echo site_url('admin/content/approvefeaturetransaction/'.$row->unique_id);?>"><?php echo lang_key('approve');?></a></li>
                            <?php }?>
                            <?php if($row->payment_type=='feature_renew') {?>
                              <li><a href="<?php echo site_url('admin/content/approvefeaturerenewtransaction/'.$row->unique_id);?>"><?php echo lang_key('approve');?></a></li>
                            <?php }?>
                        <?php }?>
                          <li><a href="<?php echo site_url('admin/content/deletetransaction/'.$row->unique_id);?>"><i class="fa fa-trash" aria-hidden="true"></i> <?php echo lang_key('delete');?></a></li>
                      <?php }?>
                      </ul>
