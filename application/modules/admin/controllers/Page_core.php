...
class Page_core extends CI_Controller {
	...
	public function add()
	{
		...
		else
		{
			...
			$data['parent'] 		= ($this->input->post('parent')==null)?0:$this->input->post('parent');
	...
	public function delete($page='0',$id='',$confirmation='')
	{
		if($confirmation=='')
		{
			...
			$post = $this->page_model->get_page_by_id($id);
			$info = '"'.$post->title.'"';
			$data['content'] = load_admin_view('confirmation_view',array('id'=>$id,'url'=>site_url('admin/page/delete/'.$page),'info'=>$info),TRUE);
			load_admin_view('template/template_view',$data);
		}
		else
		{
