...
class Users_core extends CI_Controller {
	...
	{
		$query = $this->users_model->get_all_user_phones();
		$this->load->dbutil();
		$data = $this->dbutil->csv_from_result($query); 

	    # Load the download helper and send the file to your desktop
        $this->load->helper('download');
        force_download('userphone.csv', $data);	
	...
	function add()
	{
		...
		if(get_settings('content_settings','enable_pricing','No')=='Yes')
	...
	public function sendemail($user_id='')
	{
		...
		if($user_id!='')
		{
			...
			$value['sel_email'] = $user->user_email;
		}
		...
		$data['content'] = load_admin_view('users/bulk_email_view',$value,TRUE);
