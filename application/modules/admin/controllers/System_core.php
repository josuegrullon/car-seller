...
class System_core extends CI_Controller {
	...
	{
		if($msg=='error')
			$value['msg'] = '<div class="alert alert-danger">'.lang_key('error_occurred').'</div>';
		$this->load->model('options_model');
	...
	public function savesitesettings($key='site_settings')
	{
		...
		{	
			...
			else
			{
				...
				$error = FALSE;
				if($this->input->post('site_mode')=='production' || $this->input->post('site_mode')=='development')
				{
					$this->load->helper('file');
					$data = read_file('./index.php');
					$data = str_replace("define('ENVIRONMENT', 'production')","define('ENVIRONMENT', '".$this->input->post('site_mode')."')", $data);
					$data = str_replace("define('ENVIRONMENT', 'development')","define('ENVIRONMENT', '".$this->input->post('site_mode')."')", $data);

					if ( ! write_file('./index.php', $data))
					{
						$this->session->set_flashdata('msg', '<div class="alert alert-success">'.lang_key('Unable to write the file').'</div>');
						$error = TRUE;
					}
					else
					{
				...
				elseif($this->input->post('site_mode')=='maintainance')
				{
					$this->load->helper('file');
					$data = '<h1>'.lang_key('maintenance_msg').'</h1>';
					if ( ! write_file('./index.html', $data))
					{
						$this->session->set_flashdata('msg', '<div class="alert alert-success">'.lang_key('Unable to write the file').'</div>');
						$error = TRUE;
					}
    ...
    function get_site_map_xml()
    {
        ...
        if($page_checked==1 or $blog_post_checked==2 or $estate_checked==3)
        {
            ...
            if($estate_checked==3){
                ...
                $query = $this->content_model->get_all_post('create_time','desc');
                ...
                if($post_array){
                    $xml_array = array_merge($xml_array,$post_array);
                }
            }
    ...
    function manual()
    {
		...
		// if ( ! write_file('./web_config/locals/'.$file_name, $yaml))
		// {
    	...
    	$handle = fopen("./web_config/tmp.txt", "r");
		if ($handle) {
			$i = 0;
    ...
    public function clearunusedimg($action='analyze')
    {
    	if($action == 'analyze')
    		$value['files'] = $this->system_model->get_unused_images();
    	elseif($action=='clear')
    	{
    		$this->system_model->get_unused_images('clear');
    		$value['msg'] = '<div class="alert alert-success">'.lang_key('unused_images_cleared').'</div>';
    	}
    	$value['action']    = $action;
    	$data['title']      = lang_key('clear_unused_img');
        $data['content']   	= load_admin_view('system/clear_img_view',$value,TRUE);
        load_admin_view('template/template_view',$data);
    } 
    // end
